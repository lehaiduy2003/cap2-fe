...
const Storage = ({ roomId }) => {
    ...
    const fetchTenantName = useCallback(async (tenantId) => {
        try {
            ...
            const response = await fetch(
                `${BASE_API_URL}/owner/get-users/${tenantId}`,
                {
                    headers: {
            ...
            try {
                ...
                console.log(
                    'Full parse failed, trying partial parse: ',
                    fullParseError,
                );
            }
    ...
    const fetchContracts = useCallback(async () => {
        ...
        try {
            ...
            if (responseData && responseData.data) {
                ...
                const transformedContracts = responseData.data.map(
                    (contract) => ({
                        id: contract.id,
    ...
    const filteredContracts = contracts.filter((contract) => {
        const isSenderMatch = contract.tenantId
            .toString()
            .includes(searchRoomName);
        const isDateMatch =
            (!startDate ||
            ...
            {loading ? (
                ...
                <div className='empty-state-container'>
                    <div className='empty-state-content'>
                        ...
                        <p>
                            Bạn có thể tạo hợp đồng mới bằng cách nhấn nút
                            &quot;Tạo mới&quot; ở trên
                        </p>
                    </div>
                </div>
                        ...
                        <tbody>
                            {currentContracts.map((contract, index) => (
                                <tr key={contract.id}>
                                    ...
                                    <td>
                                        {tenantNames[contract.tenantId] ||
                                            'Đang tải...'}
                                    </td>
                                    {/* <td>{contract.roomId}</td> */}
                                    <td>
                                        {roomNames[contract.roomId] ||
                                            'Đang tải...'}
                                    </td>
                                    <td>
                                        {new Date(
                                    ...
                                    <td>
                                        {contract.price_per_month?.toLocaleString()}{' '}
                                        VNĐ
                                    </td>
                                    <td>
                                        <span
